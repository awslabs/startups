...
| Security baseline          | Not included      | `baseline.tf` (GuardDuty, CloudTrail, IMDSv2, EBS encryption, budget alerts). Always emitted by `heroku-to-aws`; `gcp-to-aws` emits it only on the infrastructure route (AI-only/billing-only runs skip it). `heroku-to-aws` additionally emits Config/Security Hub controls for soc2/pci/hipaa/fedramp |
