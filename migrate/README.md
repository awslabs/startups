...
- **Generates production-ready Terraform** — `vpc.tf`, `compute.tf`, `database.tf`, `security.tf`, `baseline.tf` with account-wide security controls (GuardDuty, CloudTrail, IMDSv2, ECR scanning, plus Config and Security Hub when `compliance` includes soc2, pci, hipaa, or fedramp), and a full `terraform/README.md`
...
| Security baseline          | Not included      | `baseline.tf`: GuardDuty, CloudTrail, IMDSv2, EBS encryption, budget alerts, plus Config and Security Hub when `compliance` includes soc2/pci/hipaa/fedramp; ECR scan-on-push where ECR repositories are generated. Always emitted by `heroku-to-aws`; for `gcp-to-aws`, only on the infrastructure-generation route (AI-only/billing-only runs skip it) |
