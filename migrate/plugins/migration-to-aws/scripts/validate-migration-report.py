...
def _validate_graviton_savings(html: str, estimation_infra: dict | None) -> list[str]:
    """Conditional check: when architecture_comparison exists in estimation-infra.json,
    the report must render savings_amount and savings_percent in the body.

    This is a scoped numeric check — not a full financial audit. It confirms
    the Graviton savings figures from the estimate appear somewhere in the
    exec-costs or appendix-costs sections of the report.
    """
    if not estimation_infra:
        return []

    arch_comparison = estimation_infra.get("architecture_comparison")
    if not arch_comparison:
        return []

    errors: list[str] = []
    body = _readability_scope(html)

    # Check savings_amount appears as a dollar-formatted value
    savings_amount = arch_comparison.get("savings_amount")
    if savings_amount is not None and not _dollar_amount_present(savings_amount, body):
        errors.append(
            f"architecture_comparison.savings_amount (${savings_amount}) not found in report — "
            "render Graviton savings in exec-costs or appendix-costs"
        )

    # Check savings_percent appears (one decimal, e.g. "16.9%" or "16.9")
    savings_percent = arch_comparison.get("savings_percent")
    if savings_percent is not None:
        pct_str = f"{float(savings_percent):.1f}"
        if pct_str not in body:
            errors.append(
                f"architecture_comparison.savings_percent ({pct_str}%) not found in report — "
                "render Graviton savings percentage in exec-costs or appendix-costs"
            )

    # Soft warning (not a hard gate): check for "additional savings" framing
    # that could mislead readers into thinking Graviton is on top of Balanced.
    # This is a documentation/style check — logged but does not cause FAIL.
    if re.search(r"additional\s+savings.*graviton", body, re.IGNORECASE):
        # NOTE: This is a style warning, not a hard error. The Graviton callout
        # should frame savings as "vs x86 equivalent" since Balanced already uses
        # Graviton pricing. Uncomment the next line to make it a hard gate:
        # errors.append("framing: avoid 'additional savings' — Graviton is within Balanced, not on top")
        pass
...
def validate_report(
    ...
    *,
    ...
    # Graviton architecture_comparison: when present, savings figures must appear in report.
    errors.extend(_validate_graviton_savings(html, estimation_infra))
