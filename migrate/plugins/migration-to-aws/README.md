...
- **Generates production-ready Terraform** — `vpc.tf`, `compute.tf`, `database.tf`, `security.tf`, and a full `terraform/README.md`. For GCP migrations, also emits `baseline.tf` with account-wide security controls (GuardDuty, CloudTrail, IMDSv2, ECR scanning). `heroku-to-aws` does not yet emit `baseline.tf` (planned); it is not gated by `tf-best-practices`, which is a separate read-only policy checker.
