...
(which would hand *every* `rw` worker, including `discover`, arbitrary Bash), `generate`
...
and, always, the worker prompt's binding instruction. Note this softens the tier *ordering*
