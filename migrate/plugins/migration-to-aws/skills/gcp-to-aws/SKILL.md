...
8. **Feedback sidebar**: After a phase completes, check if feedback is due (see rules below). This runs **before** advancing to the next phase.
   ...
   - **After Estimate**: First present the post-Estimate **Decision gate** per
     ...
     [B] No thanks"
     - If user picks **A** → Load `references/phases/feedback/feedback.md`, execute it, then continue per the gate choice (Generate for C; done for A).
     - If user picks **B** → Use the Phase Status Update Protocol to set `phases.feedback` to `"completed"`. Continue per the gate choice.
