...
8. **Feedback sidebar**: After a phase completes, check if feedback is due (see rules below). This runs **before** advancing to the next phase.
   ...
   - **After Discover** (if `phases.feedback` is `"pending"`): Output to user:
     "Would you like to share quick feedback (one chat question + a short 5-question form + anonymized usage data) to help improve this tool? Your data never includes resource names, file paths, or account IDs.
     [A] Send feedback now
   ...
   - **After Estimate**: First present the post-Estimate **Decision gate** per
     ...
     - **After gate C** (continuing to Generate), use the standard form:
       "Would you like to share quick feedback now? (one chat question + a short 5-question form + anonymized usage data)
