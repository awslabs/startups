...
| Q5    | PROPOSED                                                                                                                         | 2 — AWS-only                         | Assuming AWS-only → full compute tree continues (GKE → EKS Auto Mode by default per Q8); if multi-cloud portability is required, all containers go to standard EKS instead (Auto Mode is AWS-specific, not used for multi-cloud)                |
...
| Q8    | PROPOSED (only when GKE present and Q5 ≠ multi-cloud)                                                                            | 1 — `eks-auto`                       | Assuming EKS Auto Mode → keep Kubernetes with AWS running the nodes (GKE Autopilot equivalent); answer "manage nodes myself" for standard EKS or "drop Kubernetes" for ECS Fargate                                                              |
