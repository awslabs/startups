...
| Across sessions | AgentCore Memory service recommended. Persists knowledge, user preferences, and interaction history across sessions. Can now ingest content directly into long-term memory via the IngestData API (no short-term event required), in addition to the existing short-term-event extraction path. For retarget path: evaluate existing memory backend migration (Redis → ElastiCache, Postgres → RDS, vector store → OpenSearch Serverless). |
