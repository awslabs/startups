...
- **CPU architecture (Graviton/ARM64):** read each compute resource's
  ...
  EKS arm64 node groups are single-arch on dev tier. **EKS Auto Mode (`eks-auto`) has no launch template** — set architecture on the NodePool/`aws_eks_node_class` (`requirements` selecting `kubernetes.io/arch: arm64`) instead; the same `# Graviton (ARM64)` comment applies.
