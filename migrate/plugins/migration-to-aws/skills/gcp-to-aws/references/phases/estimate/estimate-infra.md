...
| Compute (EKS Auto Mode)     | EKS control plane fixed monthly (~$73/mo = $0.10/cluster-hr × 730) + underlying EC2 instance rate × 730 hours × node count + **Auto Mode management fee** (a per-instance-type surcharge EKS adds on top of the EC2 rate, billed per-second, 1-min minimum, independent of EC2 purchase option — see `pricing-cache.md` → `### EKS`). Source it via the `awspricing` MCP (tier 2) for the exact per-instance-type rate; if MCP is unavailable, set `pricing_source: "unavailable"` for this fee, add it to `services_with_missing_fallback`, and warn that the estimate excludes the Auto Mode surcharge — do not silently drop it and do not substitute a hardcoded rate or percentage. Price the EC2 rate for the node instance family — **amd64 by default** (the built-in `general-purpose` NodePool is amd64-only); use a Graviton rate only if the arm64 custom NodePool is actually applied (see `generate-artifacts-infra.md`). Size node count from the source GKE cluster (or a dev-tier floor of 2 when unknown). Note in the estimate that Auto Mode nodes are dynamically provisioned by Karpenter, so this is a baseline, not a cap. | `aws_config` (target node instance type, node count), source `google_container_cluster` sizing |
| Compute (EKS standard)      | EKS control plane fixed monthly (~$73/mo) + EC2 rate for the managed-node-group `instance_type` × 730 hours × node count (no Auto Mode management fee).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `aws_config.instance_type`, node count                                                         |
...
| **GKE → EKS** (keep Kubernetes; Auto Mode default) | **Partial**                                            | Default target is **EKS Auto Mode** (AWS operates the nodes) — Compute Savings Plans apply to the underlying **EC2 instances** Auto Mode provisions, but **not** the Auto Mode management fee or the EKS control plane fee (~$0.10/hr per cluster). Standard node groups (`eks-standard`) and EKS Fargate profiles are also Savings-Plan-eligible on their EC2/Fargate compute. Post-migration baseline still recommended after right-sizing. |
