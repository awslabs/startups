...
| `kubernetes`                | Q8                 | How do you want to run your Kubernetes workloads on AWS?           | `[value]` selects the GKE compute target: `eks-auto` (EKS Auto Mode — default), `eks-standard` (EKS managed node groups), or `ecs-fargate` (drop Kubernetes)                                                                                                            |
