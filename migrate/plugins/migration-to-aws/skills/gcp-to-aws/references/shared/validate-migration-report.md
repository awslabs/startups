...
## Conditional checks (active when estimation data passed)

These checks are scoped numeric validations — not a full financial audit. They activate only when the corresponding data block exists in the passed estimation JSON.

| #  | Check                    | Activates when                                                                     | PASS when                                                                                     |
| -- | ------------------------ | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| C1 | Graviton savings amount  | `--estimation-infra` passed AND JSON has `architecture_comparison.savings_amount`  | Dollar-formatted `savings_amount` value appears in report body (exec-costs or appendix-costs) |
| C2 | Graviton savings percent | `--estimation-infra` passed AND JSON has `architecture_comparison.savings_percent` | `savings_percent` value (one decimal, e.g. `16.9`) appears in report body                     |

**Style guidance (soft, not gated):** The Graviton callout should frame savings as "vs x86 equivalent of the same mapping" — Balanced-tier pricing already uses Graviton rates. Avoid "additional savings on top of Balanced" framing. This is documented but not enforced as a hard gate to avoid fragile regex matching.
