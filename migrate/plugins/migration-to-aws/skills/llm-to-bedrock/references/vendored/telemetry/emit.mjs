#!/usr/bin/env node
// Migration telemetry emitter. Claude Code and Cursor use host hooks; other
// agents invoke the same emitter at the skill's state boundaries:
...
//   node emit.mjs --reconcile --via cli   other agents: explicit state reconciliation
...
function pluginVersion() {
  // The sync tool derives this sidecar from the plugin manifest and vendors it
  // with the emitter, so a standalone skill retains its distribution version.
  const manifest = readJson(path.join(path.dirname(fileURLToPath(import.meta.url)), "version.json"));
...
function runConsentCommand(action) {
  ...
  // Mutate the same record that controls reads and emission. Writing only a
  // project record cannot revoke an existing, higher-priority global grant.
  const machineFile = machineConsentFiles().find((candidate) => readJson(candidate)?.consent);
  const file = machineFile ?? path.join(migrationRoot, "telemetry.json");
  const record = readJson(file);
  const write = (consent) => {
    if (!machineFile && !existsSync(migrationRoot)) {
    ...
    writeJson(file, {
      ...record,
      consent,
      installId: asUuid(record?.installId) ?? getInstallId(),
...
async function processRun(runDir, { sessionId, sessionEndMode, endpoint }) {
  ...
  try {
    ...
    // Invocation metadata only. Host routing selects hook versus CLI; the
    // snapshot diff suppresses already processed transitions on resume.
    writeJson(snapshotFile, {
      runId,
...
async function main() {
  ...
  // Consent commands remain available on every host. Reporting from a skill
  // must never compete with the hooks registered for Claude Code and Cursor.
  const cliMode = viaMode() === "cli";
  if (cliMode && hostSource() !== "OTHER") return;
  ...
  // A CLI invocation uses cwd and must not wait for a hook payload on stdin.
  const payload = cliMode ? null : await readStdin();
