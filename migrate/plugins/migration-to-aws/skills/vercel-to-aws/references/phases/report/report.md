...
_postconditions:
  - _check_file_exists: migration-report.html
    _on_failure: _halt_and_inform
...
Renders `migration-report.html` from every upstream artifact, then runs it
...
`migration-report.html`, applying the outcome-based filter/reframe rule
...
- The retry-cap loop: up to 2 additional regeneration attempts on
  fail-with-errors, renaming to `migration-report.incomplete.html` on each
...
`migration-report.html` exists and that the validator exited 0 within the
retry cap, then emit `GATE_FAIL` or
`HANDOFF_OK | phase=report | artifacts=migration-report.html` and advance.
...
`migration-report.html` directly in the founder's default browser: run
`open "$MIGRATION_DIR/migration-report.html"` on macOS or
`xdg-open "$MIGRATION_DIR/migration-report.html"` on Linux. This is a
...
**This phase covers rendering and validating `migration-report.html` ONLY.**
