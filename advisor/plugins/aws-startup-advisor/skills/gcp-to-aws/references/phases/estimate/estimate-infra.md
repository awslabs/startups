...
| Compute (EKS Auto Mode)     | EKS control plane fixed monthly (~$73/mo = $0.10/cluster-hr × 730) + underlying EC2 instance rate × 730 hours × node count + **Auto Mode management fee** (a per-instance surcharge EKS adds on top of the EC2 rate for Auto Mode-managed nodes; billed per-second, 1-min minimum — look it up via the EKS Auto Mode pricing page/MCP, do not hardcode a stale %). Price the EC2 rate for the node instance family — **amd64 by default** (the built-in `general-purpose` NodePool is amd64-only); use a Graviton rate only if the arm64 custom NodePool is actually applied (see `generate-artifacts-infra.md`). Size node count from the source GKE cluster (or a dev-tier floor of 2 when unknown). Note in the estimate that Auto Mode nodes are dynamically provisioned by Karpenter, so this is a baseline, not a cap. | `aws_config` (target node instance type, node count), source `google_container_cluster` sizing |
