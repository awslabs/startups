...
- **Cloud Run min instances:** `google_cloud_run_v2_service` uses `scaling.min_instance_count` (or top-level `min_instance_count`). `google_cloud_run_service` (v1) does not have that field at all — its minimum-instance setting is an annotation: `template.metadata.annotations["autoscaling.knative.dev/minScale"]` or the service-level `metadata.annotations["run.googleapis.com/minScale"]`. Read whichever is present and treat its integer value (annotation values are strings, e.g. `"50"`) as `min_instance_count` for the threshold below.
- **GKE node count:** a pool can size itself three different ways — `initial_node_count`/`node_count` (fixed, no autoscaling block), the per-zone `autoscaling.min_node_count`/`max_node_count`, or the cluster-wide `autoscaling.total_min_node_count`/`total_max_node_count` (mutually exclusive with the per-zone fields, same node-pool resource). Extract whichever form is present into inventory `config` and compare all of them against the same threshold — a fixed `node_count = 20` with no autoscaling block is exactly as authored-large as `max_node_count = 20`.
...
| `google_container_cluster` / `google_container_node_pool`                                | 2× `t4g.small`                       | `machine_type` present and does **not** match `*micro*` or `*small*`; any of `initial_node_count`, `node_count`, `min_node_count`, `total_min_node_count`, or `gke_node_count` > 2; any of `max_node_count` or `total_max_node_count` > 2 |
...
| `google_container_node_pool` with `node_count = 20`, `machine_type = "e2-small"`, no `autoscaling` block                                       | node count = 20                 | Yes         |
| `google_container_node_pool` with `autoscaling { total_min_node_count = 1, total_max_node_count = 20 }`                                        | max node count = 20             | Yes         |
| `google_container_node_pool` with `autoscaling { min_node_count = 1, max_node_count = 2 }`                                                     | max node count = 2              | No          |
