...
- **GKE node count:** evaluate **every** node pool, whether declared as a standalone `google_container_node_pool` resource or as an inline `node_pool { ... }` block (or the default pool) inside `google_container_cluster` — inline blocks are not separate resources, so a per-resource scan misses them. A pool can size itself three ways — `initial_node_count`/`node_count` (fixed, no autoscaling block), the **per-zone** `autoscaling.min_node_count`/`max_node_count`, or the **cluster-wide** `autoscaling.total_min_node_count`/`total_max_node_count` (mutually exclusive with the per-zone fields). The per-zone and cluster-wide forms are not the same unit: per-zone counts are multiplied by the number of zones the pool spans. **Before comparing, convert per-zone counts to totals:** `effective_total = per_zone_count × zone_count`, where `zone_count` is the length of the pool's effective `node_locations` (the pool's own `node_locations`, else the cluster's; default to 1 only when neither is authored). `node_count`/`initial_node_count` and the `total_*` fields are already totals — use them as-is. Compare the resulting total against the threshold, so a fixed `node_count = 20`, a cluster-wide `total_max_node_count = 20`, and a per-zone `max_node_count = 2` across three `node_locations` (= 6) are each judged on true node count rather than the author's chosen form.
...
| `google_container_cluster` / `google_container_node_pool` (each pool, standalone or inline) | 2× `t4g.small`                       | `machine_type` present and does **not** match `*micro*` or `*small*`; any fixed/total count (`initial_node_count`, `node_count`, `total_min_node_count`, `total_max_node_count`, `gke_node_count`) > 2; **or** a per-zone count (`min_node_count` / `max_node_count`) whose zone-normalized total (`× node_locations` count, see above) > 2 |
...
| `google_container_node_pool` with `node_count = 20`, `machine_type = "e2-small"`, no `autoscaling` block                                       | total node count = 20           | Yes         |
