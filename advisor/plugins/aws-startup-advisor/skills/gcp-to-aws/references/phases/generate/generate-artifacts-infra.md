...
| `baseline.tf`   | security baseline | Account-wide security baseline: alternate contacts, password policy, S3 PAB, EBS encryption, Access Analyzer, IMDSv2 default, CloudTrail + S3 log bucket, AWS Budget, GuardDuty. Plus a compliance-conditional section (Config + Security Hub + standards) when `design_constraints.compliance.value` contains soc2/pci/hipaa/fedramp. Always emitted; users who want to skip it can delete this file before `terraform apply`. |
...
> `baseline.tf` is always emitted. It is NOT driven by `aws-design.json` clusters — the resources are workload-independent account controls. The compliance-conditional subset (Config + Security Hub) is emitted within the same file when `design_constraints.compliance.value` contains soc2/pci/hipaa/fedramp. The `aws_budgets_budget` resource reads `estimation-infra.json` to set its `limit_amount`. See Step 1.5 below. Users who want to skip the baseline can delete `terraform/baseline.tf` before `terraform apply`.
...
- **CPU architecture (Graviton/ARM64):** read each compute resource's
  ...
  EKS arm64 node groups are single-arch on dev tier. **EKS Auto Mode (`eks-auto`) — Graviton is NOT a Terraform flag.** The built-in `general-purpose` NodePool is **amd64-only and immutable**, and `aws_eks_cluster` cannot express node architecture, so arm64 app workloads require an **Auto Mode NodePool manifest that references the built-in `default` NodeClass** applied post-provision (see the EKS branch in Step 3.1). When `graviton.target_architecture == arm64` on an Auto Mode cluster, emit that manifest (`k8s/nodepool-arm64.yaml`) plus a `kubectl apply` step in the runbook; if the run cannot emit/apply Kubernetes manifests, set the cluster to **amd64**, skip the arm64 image build, and flag Graviton as not applied — do **not** report arm64.
