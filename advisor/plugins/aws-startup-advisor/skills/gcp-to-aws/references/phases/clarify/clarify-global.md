...
| Answer                    | Recommendation Impact                                                                                                                 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Yes, multi-cloud required | **Immediate Standard EKS Cluster recommendation** — Kubernetes is the only portable abstraction layer. Skip Q8. ECS Fargate excluded. |
| No, AWS-only acceptable   | Full compute decision tree continues — EKS Auto Mode vs Standard EKS Cluster vs ECS Fargate evaluated based on K8s preference (Q8)    |

Interpret:

```
1 -> compute: "eks" — Immediate Standard EKS Cluster recommendation (kubernetes: "eks-standard"). EKS Auto Mode is NOT used for multi-cloud — its node management is AWS-specific and defeats portability; a Standard EKS Cluster is the portable target. EARLY EXIT: skip Q8.
