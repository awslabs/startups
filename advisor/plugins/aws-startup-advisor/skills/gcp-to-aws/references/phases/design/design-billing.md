...
| `google_container_cluster`       | GKE                  | EKS Auto Mode                          | EKS standard node groups (`kubernetes: eks-standard`), ECS Fargate (`kubernetes: ecs-fargate`) — Auto Mode is the AWS-only default; billing-only mode cannot detect Autopilot vs Standard, so use the Q8 preference when present, else Auto Mode. **Under `compute: "eks"` (Q5 = multi-cloud), use EKS standard node groups, not Auto Mode** (Auto Mode's node management is AWS-specific and defeats portability). |
| `google_app_engine_application`  | App Engine           | Elastic Beanstalk                      | Fargate (`compute_model: container_orchestration`), Lambda (`serverless`); EKS standard node groups under `compute: "eks"` (Auto Mode not used for multi-cloud) — see multi-cloud override below                                                                                                                                                                                                                    |
...
**App Engine multi-cloud override (applies even in billing mode):** If `gcp_service_type` is `google_app_engine_application` **and** `design_constraints.compute.value == "eks"` (Q5 = multi-cloud), assign **EKS standard node groups** instead of the Elastic Beanstalk default — the portability override is a top-level design constraint, not a configuration detail, so it applies on the billing path too (same override that forces GKE → standard EKS under multi-cloud). **Auto Mode is not used for multi-cloud** — its node management is AWS-specific and defeats portability. Set rationale to: "Billing heuristic: App Engine → EKS standard node groups (multi-cloud portability required — overrides Elastic Beanstalk default; Auto Mode excluded for portability). Provide Terraform files for configuration-aware mapping." `confidence: billing_inferred`.
