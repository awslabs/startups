...
- **Auto Mode regional availability (gate before finalizing `eks-auto`):** validate EKS Auto Mode is available in the target region (`preferences.json → design_constraints.target_region`) via `get_regional_availability` from the `awsknowledge` MCP server — the same source-of-truth check the repo uses for AgentCore/Bedrock (see `references/shared/ai-migration-guardrails.md`). Auto Mode is broadly available in commercial regions, but **verify GovCloud/opt-in partitions** — a `fedramp`/GovCloud target (Q2) is the likely gap. If Auto Mode is **not** available in the target region, **fall back to `eks-standard`** (standard EKS managed node groups run anywhere EKS is available) in the **same region** — do not switch regions (Q1/compliance fixed it) and do not silently emit an Auto Mode cluster that will not deploy. Record the fallback in the mapping `rationale` and a `warnings[]` entry, and tell the user: "EKS Auto Mode isn't available in `<region>` — using standard EKS managed node groups instead." If the MCP call fails, proceed with `eks-auto` but add a `warnings[]` note to verify Auto Mode availability in the target region before applying.
...
2. **Operational Model**: Managed (Lambda, Fargate, **EKS Auto Mode** — AWS operates the nodes) vs Self-Hosted (EC2, **standard EKS managed node groups**)?
   ...
   - For App Engine sources: Elastic Beanstalk (PaaS-to-PaaS) when `compute_model` is absent or `"managed_platform"` **and `compute` ≠ `"eks"`** (when `compute: "eks"`, the criterion-1 override already selected EKS with standard node groups)
