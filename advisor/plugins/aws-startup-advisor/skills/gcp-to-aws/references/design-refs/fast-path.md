...
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Containerized workloads | Fargate (default for non-K8s containers, e.g. Cloud Run), Lambda (event-driven); **GKE / Kubernetes workloads → EKS with standard node groups when `compute: "eks"` (Q5 multi-cloud) or on explicit Q8 opt-in (`kubernetes: "eks-standard"`); otherwise EKS Auto Mode (default)** | Deeper integration with VPC, ALB, IAM, and auto-scaling than lighter-weight alternatives; Auto Mode is AWS's recommended low-ops Kubernetes model and the GKE Autopilot equivalent |
...
**Enforcement exemption (App Engine + `compute: "eks"` / Q5 = multi-cloud):** When `compute: "eks"` is set (Q5 = multi-cloud), Q7b does not fire and App Engine routes to **EKS (Standard Cluster)** via the container path (see `compute.md` App Engine section). **Do not** apply the PaaS row to re-substitute Elastic Beanstalk in that case — the PaaS row's condition (`compute` ≠ `"eks"`) is already false, so it does not match. This is the same top-level portability override that forces GKE to EKS.
