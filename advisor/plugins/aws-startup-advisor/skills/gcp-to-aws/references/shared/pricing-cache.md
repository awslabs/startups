...
### OpenAI on Bedrock (GPT-6 Astra, GPT-5.x, and gpt-oss)
...
a same-model GPT-5.x migration in-region costs about 10% MORE, not the same. Do not describe it as cost-neutral.
For Astra, verify the source provider's rate separately before claiming same-model savings or parity.
...
#### GPT-5.x models — short context (272K), in-region
