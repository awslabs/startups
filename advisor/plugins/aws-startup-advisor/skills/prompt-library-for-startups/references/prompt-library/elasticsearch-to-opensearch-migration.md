...
Stage 4 - Dashboards & Monitoring
Stage 5 - Security
Stage 6 - Cutover & Validation
</overall_steps>
...
<step6>
This stage covers the final cutover from Elasticsearch to OpenSearch and post-migration validation. Please write the cutover and validation strategy referring to the following.

1. Pre-Cutover Readiness Checklist
Information:
* Migration window constraints (from input section 6)
* Service criticality levels
* Completion status of Stages 1–5
Provide:
* Go/no-go criteria checklist
* Rollback plan with specific triggers and procedures
* Communication plan for stakeholders

2. Traffic Cutover Execution
Information:
* Client application list and connection methods
* DNS/load balancer configuration
* Migration window constraints per service
Provide:
* Step-by-step cutover procedure (blue-green or rolling)
* Client endpoint switchover commands
* Timing coordination across dependent services
* Rollback execution steps if issues are detected

3. Post-Migration Validation
Information:
* Expected document counts per index
* Key queries and expected response times
* SLA requirements
Provide:
* Data integrity verification queries (document counts, checksums)
* Performance benchmark comparisons (source vs. target)
* Functional test procedures for critical search/analytics workflows
* Alerting and monitoring confirmation in the new cluster

4. Decommissioning Source Cluster
Information:
* Source cluster resource inventory
* Backup retention requirements
Provide:
* Final snapshot of source cluster (safety net)
* Resource cleanup sequence and timeline
* Cost savings validation after decommission
</step6>
</detailed_step>
