...
`{SKILL_LABEL}` = "Azure" — probe once, prefer `build_estimate` on the
scenario's Balanced-tier services (per `estimate-infra.md` § Scenarios — the
anchor tier), store the URL as `estimation_summary.calculator_url`, null +
one chat note on any failure.
