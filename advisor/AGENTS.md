...
### `claude-platform-evaluation` — choose a Claude access path

- Qualifies Anthropic API, Claude Platform on AWS, and Claude on Amazon Bedrock against a startup's must-haves, acceptable alternatives, preferences, and unknowns.
- Triggered by _"Anthropic API vs Bedrock"_, _"should we use Claude Platform on AWS"_, _"where should we run Claude"_, or requests for a small cross-path validation plan.
- Stops at access-path selection. SDK rewrites route to `llm-to-bedrock`; AWS agent-runtime selection routes to `agent-advisor`.
...
- Claude access-path selection before a migration or runtime decision routes to `claude-platform-evaluation`.
